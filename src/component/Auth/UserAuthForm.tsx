import { getCandidateProfile, getOnboardingStatus, loginUser } from "@/api/services";
...
const UserAuthForm = () => {
  ...
  const { data: profileData, refetch: fetchProfile, isSuccess } = useQuery({
    queryKey: ["profile"],
  ...
  useEffect(() => {
    ...
    if (isSuccess && token && profileData && !profileData?.candidate?.isEmailVerified) {
